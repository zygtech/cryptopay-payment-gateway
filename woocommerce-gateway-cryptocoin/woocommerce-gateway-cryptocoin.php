...
function woocommerce_cryptocoin_init(){
  ...
  class WC_Cryptocoin extends WC_Payment_Gateway{
	...
	if ($woocommerce -> cart!=null) $woocommerce -> cart -> empty_cart();
        return array('result' => 'success', 'redirect' => 'http://domain.com/pay.php?id=' . $order_id . '&site=' . get_site_url() . '&amount=' . $order -> order_total . '&my_address=' . base64_encode($this -> my_address) . '&hash=' . md5($order_id . get_site_url() . $order -> order_total . $this -> my_address . 'CryptoCoin')
        );
    }
